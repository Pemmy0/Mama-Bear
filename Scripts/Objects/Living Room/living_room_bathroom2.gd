...
func _ready():
	...
	if !GlobalPosition.to_living_from_bathroom:
		load_sfx(door_close)
		audio_stream_player.pitch_scale += randf_range(-0.2, 0.2)
		audio_stream_player.play()
		player.animated_sprite_2d.flip_h = true
		player.position.x = GlobalPosition.bathroom_door_position.x
		GlobalPosition.to_living_from_bathroom = true
	pointer.hide()
...
func scene_transition():
	...
	get_tree().change_scene_to_file("res://Scenes/Main Scenes/scene_4_bathroom.tscn")
